...
// ====== Seasonal + Daily Greeting (v8.7D) ======
function seasonFor(month){ if (month==11 || month<=1) return 'winter'; if (month>=2 && month<=4) return 'spring'; if (month>=5 && month<=7) return 'summer'; return 'autumn'; }
const CLASS_LINES = {
  Ranger:["The city hides trails only a Ranger can uncover.","Your boots remember paths the map forgot.","A quiet street hums with side-quests."],
  Druid:["Green things grow where your focus lands.","Patience is your greatest spell today.","A tea, a breath, a tiny ritual—progress."],
  Bard:["Every errand has a rhythm. Find it.","Your words are small spells. Cast kindly.","A good tale needs one brave scene."],
  Barbarian:["The world is stubborn. So are you.","Lift something heavier than your doubts.","Courage first, clarity next."],
  Rogue:["Slip between distractions like shadow.","Small heists of time win the day.","Plan, then pounce."]
};
const SEASON_LINES = {
  winter:["Frost sharpens the morning. Keep the fire inside.","Quiet streets; loud resolve.","Snow hushes the city. Listen for your cue."],
  spring:["Rain polishes the day; new ideas sprout.","Blossoms and beginnings. Start small.","The air says ‘again’. Answer it."],
  summer:["Light spills everywhere; pace yourself.","Heat forgives nothing—hydrate and strike.","Long days, long XP bars—chip away."],
  autumn:["Leaves drift like unclaimed quests.","Crisp air, clear choices.","Harvest the easy wins first."]
};
function pick(arr){ return arr[Math.floor(Math.random()*arr.length)]; }
function updateDailyGreeting(){
  const today = new Date(); const key = today.toDateString();
  if (state.greeting?.date === key && state.greeting.text) return;
  const cls = state.hero?.cls || 'Ranger'; const s = seasonFor(today.getMonth());
  const lineA = pick(CLASS_LINES[cls] || CLASS_LINES.Ranger); const lineB = pick(SEASON_LINES[s]);
  const text = lineB + " " + lineA;
  const whisperPool = ["Mark one quest as done before noon.","Two minutes of tidying earns surprise XP.","If it takes under 5 minutes, do it now.","One call, one message, one stretch."];
  const whisper = pick(whisperPool);
  state.greeting = { date: key, text, whisper }; save();
}
function renderGreetingCard(){
  const el = document.querySelector('#dmGreeting'); if(!el) return;
  const name = state.hero?.name || 'Adventurer'; const cls = state.hero?.cls || 'Ranger';
  el.innerHTML = `<div class="dm-card"><div class="dm-title">${name} the ${cls}</div><div class="dm-body">${state.greeting.text||''}</div><div class="dm-foot">Daily greeting • ${state.greeting.date||''}</div></div>`;
}
function renderWhisper(){ const w = document.querySelector('#dmWhisper'); if(w) w.textContent = state.greeting.whisper || ''; }

function renderAll(){ applyTheme(); renderHeader(); updateDailyGreeting(); renderGreetingCard(); renderWhisper(); renderStats(); renderQuests(); }
...
document.addEventListener('DOMContentLoaded', () => {
  try { load(); bind(); renderAll(); 
    const b=document.getElementById('debugBanner'); if(b){b.style.display='inline-block'; b.textContent='JS ready: buttons bound';}
  } catch(e){ const b=document.getElementById('debugBanner'); if(b){b.style.display='inline-block'; b.style.background='#ef4444'; b.textContent='Fatal: '+e.message;} console.error(e); }
});
